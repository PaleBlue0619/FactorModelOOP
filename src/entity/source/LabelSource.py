...
from src.entity.source.DataSource import Source
...
class LabelSource(Source):
    ...
    def getDateListFromLabel(self, startDate: pd.Timestamp, endDate: pd.Timestamp, labelName: str) -> List[pd.Timestamp]:
        ...
        select min(tradeDate) as min_date, 
                ...
                where startDate<={self.labelDateCol} and {self.labelDateCol1}>=startDate 
                and {self.labelDateCol1}<endDate and label == "{labelName}"
        """)
